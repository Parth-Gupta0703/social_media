...
class _ModerationCardState extends State<ModerationCard> {
  ...
  Color _reasonColor(String reason) {
    ...
    if (t.contains('hate') || t.contains('abuse')) return const Color(0xFFFF6B6B);
    if (t.contains('violence') || t.contains('threat')) return const Color(0xFFEE5A24);
    if (t.contains('explicit') || t.contains('adult')) return const Color(0xFFFF9F43);
  ...
  IconData _reasonIcon(String reason) {
    ...
    if (t.contains('hate') || t.contains('abuse')) return Icons.sentiment_very_dissatisfied_rounded;
    if (t.contains('violence') || t.contains('threat')) return Icons.dangerous_rounded;
    if (t.contains('explicit') || t.contains('adult')) return Icons.no_adult_content_rounded;
  ...
  Future<void> _onDelete() async {
    ...
    try {
      ...
      if (originalPostId != null && originalPostId.isNotEmpty) {
        final postRef = db.collection('User Posts').doc(originalPostId);
      ...
      await db.collection('Admin Activity').add({
        'Action': 'delete',
        'AdminEmail': FirebaseAuth.instance.currentUser?.email ?? 'unknown_admin',
  ...
  Future<void> _onDismiss() async {
    ...
    final confirmed = await showModerationConfirmDialog(
      ...
      body: 'The report is a false alarm. The content stays live and the report is removed from the queue.',
                      ...
                      children: [
                        ...
                        Text(
                          ...
                          style: const TextStyle(
                              color: Color(0xFF677489), fontSize: 12),
                          maxLines: 1,
                ...
                children: [
                  ...
                  if (hasParentContext) ...[
                    ...
                    OutlinedButton.icon(
                      ...
                      label: const Text('View Parent Thread Context',
                          style: TextStyle(
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              ...
                              style: TextStyle(
                                  fontSize: 11, color: Color(0xFFB05A00)),
                            ),
                          ),
                        ],
                      ),
